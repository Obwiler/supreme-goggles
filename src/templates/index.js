...
// ─── 便捷获取函数 ───────────────────────────────────────
function getAllEffectTemplates() {
    return Object.values(effectTemplates);
}
function getAllConditionTemplates() {
    return Object.values(conditionTemplates);
}
function getAllCostTemplates() {
    return Object.values(costTemplates);
}
function getEffectTemplate(type) {
    return effectTemplates[type];
}
function getConditionTemplate(type) {
...
export { 
// 注册表
effectTemplates, conditionTemplates, costTemplates, 
// 函数
getAllEffectTemplates, getAllConditionTemplates, getAllCostTemplates, getEffectTemplate, getConditionTemplate, getCostTemplate, 
// 独立模板导出（供按需引用）
dealDamageEffect, restoreLifeEffect, gainArmorEffect, loseArmorEffect, gainEnergyEffect, loseEnergyEffect, drawCardEffect, discardCardEffect, addMarkEffect, removeMarkEffect, modifyAttributeEffect, movePositionEffect, switchPositionEffect, summonEffect, destroyEffect, stunEffect, silenceEffect, tauntEffect, stealthEffect, copyCardEffect, transformEffect, discoverEffect, recycleEffect, delayedEffect, hasMarkCondition, attributeCheckCondition, cardInHandCondition, unitCountCondition, positionCheckCondition, turnCheckCondition, randomChanceCondition, comboCheckCondition, equipmentCheckCondition, aliveCheckCondition, distanceCheckCondition, energyCheckCondition, payLifeCost, payEnergyCost, payArmorCost, discardCardCost, destroyEquipmentCost, consumeMarkCost, skipDrawCost, sacrificeUnitCost };
